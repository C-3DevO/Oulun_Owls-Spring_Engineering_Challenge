...
static void configure_cli11_scheduler_policy_args(CLI::App& app, std::optional<scheduler_policy_config>& policy_cfg)
{
  ...
  static time_ai_scheduler_config ai_cfg;
  ...
  // AI scheduler (OUR ADDITION)
  CLI::App* ai_sched_cfg_subcmd =
      add_subcommand(app, "ai_sched", "AI-based scheduler configuration")->configurable();

  ai_sched_cfg_subcmd->parse_complete_callback([&]() {
    if (app.get_subcommand("ai_sched")->count() != 0) {
      policy_cfg = ai_cfg;
    }
  });
}
