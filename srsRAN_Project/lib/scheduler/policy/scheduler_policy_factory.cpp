...
std::unique_ptr<scheduler_policy> srsran::create_scheduler_strategy(
    const scheduler_ue_expert_config& expert_cfg_,
    du_cell_index_t                   cell_index)
{
  if (std::holds_alternative<time_ai_scheduler_config>(expert_cfg_.policy_cfg)) {
    return std::make_unique<scheduler_time_ai>(expert_cfg_, cell_index);
  }
