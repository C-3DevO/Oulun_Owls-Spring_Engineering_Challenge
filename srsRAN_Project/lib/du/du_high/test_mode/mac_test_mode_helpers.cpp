...
static unsigned get_dynamic_test_cqi(rnti_t rnti)
{
  struct cqi_state {
    unsigned cqi = 10;
                          ...
                          const du_test_mode_config::test_mode_ue_config&                 test_ue_cfg)
{
  ...
  unsigned cqi = get_dynamic_test_cqi(rnti);
  payload.push_back(cqi, CQI_BITLEN);
  //fmt::print("Test UE RNTI={} dynamic CQI={}\n", to_value(rnti), cqi);
}
