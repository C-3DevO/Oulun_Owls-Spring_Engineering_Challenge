...
static double compute_jain(const double* x, int n)
{
  double sum = 0.0;
  double sum_sq = 0.0;
...
static int is_dl_throughput_metric(const char* name)
{
  return strstr(name, "DRB.UEThpDl") != NULL ||
         strstr(name, "UEThpDl") != NULL;
}

static void print_fairness_report(fairness_ue_t* ues, int ue_count)
{
  double thp[MAX_UE_FAIRNESS] = {0};
  double sum = 0.0;
  ...
  double avg  = (ue_count > 0) ? sum / ue_count : 0.0;
  double jain = compute_jain(thp, ue_count);

  printf("\n[FAIRNESS] active_ues=%d avg_dl=%.3f jain=%.4f\n", ue_count, avg, jain);

  int best_idx = -1;
  double best_deficit = -1.0;

  for (int i = 0; i < ue_count; ++i) {
    double deficit = max_double(0.0, avg - ues[i].thp_dl);

    printf("[FAIRNESS] UE=%ld thp_dl=%.3f deficit=%.3f\n",
           ues[i].ue_id, ues[i].thp_dl, deficit);
  ...
  if (best_idx >= 0) {
    printf("[FAIRNESS] PRIORITIZE UE=%ld deficit=%.3f\n\n",
           ues[best_idx].ue_id, best_deficit);
  }
}
//
...
{
  printf("\n[FAIRNESS DEBUG] sm_cb_kpm entered\n");
  fflush(stdout);
  assert(rd != NULL);
  assert(rd->type == INDICATION_MSG_AGENT_IF_ANS_V0);
  ...
  printf("[FAIRNESS DEBUG] kpm->msg.type = %d\n", kpm->msg.type);
  fflush(stdout);
  kpm_ric_ind_hdr_format_1_t const* hdr_frm_1 = &kpm->hdr.kpm_ric_ind_hdr_format_1;

  int64_t now = time_now_us();
  //
  fairness_ue_t fairness_ues[MAX_UE_FAIRNESS] = {0};
  int fairness_ue_count = 0;
  //

  {
    ...
    if (kpm->msg.type == FORMAT_1_INDICATION_MESSAGE) {
      printf("[FAIRNESS DEBUG] FORMAT_1_INDICATION_MESSAGE received\n");
      fflush(stdout);
      ...
      printf("[FAIRNESS DEBUG] FORMAT_3_INDICATION_MESSAGE received\n");
      fflush(stdout);
      kpm_ind_msg_format_3_t const* msg_frm_3 = &kpm->msg.frm_3;
      printf("[FAIRNESS DEBUG] msg_frm_3->ue_meas_report_lst_len = %zu\n", msg_frm_3->ue_meas_report_lst_len);
      fflush(stdout);
      ...
      for (size_t i = 0; i < msg_frm_3->ue_meas_report_lst_len; i++) {
        ...
        switch (msg_frm_3->meas_report_per_ue[i].ue_meas_report_lst.type)
        {
          case GNB_UE_ID_E2SM:
            if (msg_frm_3->meas_report_per_ue[i].ue_meas_report_lst.gnb.gnb_cu_ue_f1ap_lst != NULL) {
              ue_id = msg_frm_3->meas_report_per_ue[i].ue_meas_report_lst.gnb.gnb_cu_ue_f1ap_lst[0];
              printf("UE ID type = gNB-CU, gnb_cu_ue_f1ap = %ld\n", ue_id);
            } else {
              ue_id = (long)msg_frm_3->meas_report_per_ue[i].ue_meas_report_lst.gnb.amf_ue_ngap_id;
              printf("UE ID type = gNB, amf_ue_ngap_id = %ld\n", ue_id);
            }
            break;

          case GNB_DU_UE_ID_E2SM:
            ue_id = (long)msg_frm_3->meas_report_per_ue[i].ue_meas_report_lst.gnb_du.gnb_cu_ue_f1ap;
            printf("UE ID type = gNB-DU, gnb_cu_ue_f1ap = %ld\n", ue_id);
            break;

          case GNB_CU_UP_UE_ID_E2SM:
            ue_id = (long)msg_frm_3->meas_report_per_ue[i].ue_meas_report_lst.gnb_cu_up.gnb_cu_cp_ue_e1ap;
            printf("UE ID type = gNB-CU-UP, gnb_cu_cp_ue_e1ap = %ld\n", ue_id);
            break;
        ...
        for (size_t j = 0; j < msg_frm_1->meas_data_lst_len; j++) {
          for (size_t z = 0; z < msg_frm_1->meas_data_lst[j].meas_record_len; z++) {
                ...
                case NAME_MEAS_TYPE:
                {
                  ...
                  switch (msg_frm_1->meas_data_lst[j].meas_record_lst[z].value)
                  {
                    case REAL_MEAS_VALUE:
                      meas_value = msg_frm_1->meas_data_lst[j].meas_record_lst[z].real_val;
                      printf("%s = %.2f\n", meas_info_name_str, meas_value);
                      break;

                    case INTEGER_MEAS_VALUE:
                      meas_value = (double)msg_frm_1->meas_data_lst[j].meas_record_lst[z].int_val;
                      printf("%s = %d\n", meas_info_name_str,
                  ...
                  if (valid_value && is_dl_throughput_metric(meas_info_name_str)) {
                    ue_dl_thp = meas_value;
                  }
            ...
            if (msg_frm_1->meas_data_lst[j].incomplete_flag &&
                *msg_frm_1->meas_data_lst[j].incomplete_flag == TRUE_ENUM_VALUE) {
              printf("Measurement Record not reliable\n");
            }
          }
        }

        if (ue_id >= 0 && ue_dl_thp >= 0.0 && fairness_ue_count < MAX_UE_FAIRNESS) {
          fairness_ues[fairness_ue_count].ue_id = ue_id;
          fairness_ues[fairness_ue_count].thp_dl = ue_dl_thp;
          fairness_ues[fairness_ue_count].has_thp_dl = 1;
          fairness_ue_count++;
        }
      }

      if (fairness_ue_count > 0) {
        print_fairness_report(fairness_ues, fairness_ue_count);
      }
      //
    } else {
      printf("[FAIRNESS DEBUG] unknown kpm ind format = %d\n", kpm->msg.type);
      fflush(stdout);
    }
